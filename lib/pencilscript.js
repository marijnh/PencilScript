(function(exports) {
  ...
  var attrProps = set(
    ...
    "lighting-color mask opacity orientation origin overflow paint-order stroke stroke-dasharray",
  ...
  function alignBox(box, node) {
    if (!node) node = box.node;
    var strokeW, half = false;
    if (getComputedStyle(node).stroke != "none" && (strokeW = strokeWidth(node)) != 0) {
    ...
    setAttrs(node, {x: box.left, y: box.top,
  ...
  exports.grid = function(conf) {
    ...
    var unitX = conf.unitX || conf.unit || 10, unitY = conf.unitY || conf.unit || 10;
    for (var x = box.left + unitX, e = box.right - mu; x < e; x += unitX) {
    ...
    for (var y = box.top + unitY, e = box.bottom - mu; y < e; y += unitY) {
      var line = svg("path", {d: "M " + box.left + " " + y + " L " + box.right + " " + y}, conf, "attr");
      box.node.appendChild(line);
      if (conf.align !== false) alignSimplePath(line);
    }
    if (!conf.open) {
      var rect = svg("rect", {x: box.left, y: box.top, width: box.width, height: box.height},
                     combine({fill: "none"}, conf), "attr");
      box.node.appendChild(rect);
      if (conf.align !== false) alignBox(box, rect);
    }
  ...
  exports.line = function(conf) {
    var line = add(svg("path", {d: conf.path || "M " + conf.startX + " " + conf.startY + " L " + conf.endX + " " + conf.endY}, conf), conf);
    if (conf.align !== false) alignSimplePath(line);
    return new Node(line);
  };

  exports.arrow = function(conf) {
    var line = exports.line(conf);
    var end = /[-\d\.]+ [-\d\.]+$/.exec(line.node.getAttribute("d"));
    exports.polygon({transform: conf.transform,
                     translate: end[0],
                     rotate: Math.atan2(conf.endY - conf.startY, conf.endX - conf.startX) / Math.PI * 180 + 90,
  ...
  exports.use = function(conf) {
    var node = svg("use", {x: conf.left, y: conf.top}, conf);
    node.setAttributeNS("http://www.w3.org/1999/xlink", "href", conf.href);
    return new Node(add(node, conf));
  };
