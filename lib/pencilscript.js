(function(exports) {
  ...
  var CurvedLine = type(Shape, null, {
    draw: function() {
      ...
      return svg("path", {d: path}, combine({fill: "none"}, this.conf));
    }
  });
  ...
  var Polygon = type(Shape, null, {
    draw: function() {
      var path = this.conf.path;
      if (!path) {
        var points = this.conf.points;
  ...
  var Text = type(Shape, null, {
    draw: function() {
      var node = svg("text", {x: this.conf.left, y: this.conf.top}, this.conf);
      node.appendChild(document.createTextNode(this.conf.text));
      return node;
    }
  });
